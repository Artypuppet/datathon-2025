"""
End-to-end test for VectorDB with AWS OpenSearch.

Tests the complete pipeline:
1. Parse and aggregate AAPL filings
2. Generate embeddings
3. Store in AWS OpenSearch
...
def check_opensearch_config():
    """Check that OpenSearch configuration is set."""
    endpoint = os.getenv('OPENSEARCH_ENDPOINT')
    if not endpoint:
        raise ValueError(
            "OPENSEARCH_ENDPOINT environment variable not set.\n"
            "Please set it: export OPENSEARCH_ENDPOINT=https://your-endpoint.es.amazonaws.com"
        )
    ...
    if not use_iam:
        ...
        if not username or not password:
            raise ValueError(
                "For basic auth, OPENSEARCH_USERNAME and OPENSEARCH_PASSWORD must be set.\n"
                "Or set OPENSEARCH_USE_IAM_AUTH=true for IAM authentication."
            )
    
    logger.info(f"[OK] OpenSearch configuration found")
    logger.info(f"  Endpoint: {endpoint}")
    logger.info(f"  Auth: {'IAM' if use_iam else 'Basic (username/password)'}")
...
def main():
    parser = argparse.ArgumentParser(description='Test VectorDB with AWS OpenSearch')
    ...
    print_section("VECTORDB OPENSEARCH TEST", f"Testing with {args.ticker} using AWS OpenSearch")
    ...
    if not args.skip_pipeline:
        ...
        # This provides targeted sentence matches with context, perfect for BERT
        ...
        orchestrator.embedding_stage = EmbeddingStage(
            ...
            context_window_sentences=3,  # Extended context window (±3 sentences)
            sentences_per_chunk=3  # Each chunk contains 3 sentences
        )
        logger.info("[INFO] Enabled sentence-level chunking with contextual enrichment")
        logger.info("[INFO] Sentences per chunk: 3")
        logger.info("[INFO] Context window: ±3 sentences")
    ...
    # Step 2: Initialize OpenSearch
    print_section("STEP 2: Initializing OpenSearch", "Connecting to AWS OpenSearch...")
    
    try:
        vectordb = get_vectordb_client(backend='opensearch')
        ...
        if args.clear_opensearch:
            ...
            try:
                ...
                logger.info(f"[OK] Deleted {deleted_count} existing embeddings for {ticker}")
        ...
        logger.error("  1. OPENSEARCH_ENDPOINT is set")
        logger.error("  2. OPENSEARCH_USERNAME and OPENSEARCH_PASSWORD are set (for basic auth)")
        logger.error("  3. Or OPENSEARCH_USE_IAM_AUTH=true with proper AWS credentials (for IAM auth)")
        return
    ...
    try:
        analyzer = LegislationImpactAnalyzer(
            # LLM analysis disabled by default (skip if short on time)
            # legislation_text=legislation_text,  # Uncomment to enable LLM analysis
            # use_llm_analysis=True,  # Uncomment to enable LLM analysis
    ...
    if 'llm_analysis' in impact_result:
        llm = impact_result['llm_analysis']
        print(f"\n{'='*80}")
        print("LLM-BASED RISK ANALYSIS:")
        print(f"{'='*80}")
        
        print(f"\nRecommendation: {llm.get('recommendation', 'neutral').upper()}")
        print(f"Confidence: {llm.get('confidence', 0)}%")
        
        if 'legislation_summary' in impact_result:
            print(f"\nLegislation Summary:")
            print(f"  {impact_result['legislation_summary']}")
        
        print(f"\nImpact Summary:")
        print(f"  {llm.get('impact_summary', 'N/A')}")
        
        print(f"\nBusiness Impact:")
        print(f"  {llm.get('business_impact', 'N/A')}")
        
        if llm.get('affected_risk_types'):
            print(f"\nAffected Risk Types:")
            for risk_type in llm['affected_risk_types']:
                print(f"  - {risk_type}")
        
        print(f"\nRecommendation Reasoning:")
        print(f"  {llm.get('recommendation_reasoning', 'N/A')}")
        
        if llm.get('rotation_target'):
            print(f"\nRotation Target: {llm['rotation_target']}")
        
        if llm.get('mitigation_strategies'):
            print(f"\nMitigation Strategies:")
            for strategy in llm['mitigation_strategies']:
                print(f"  - {strategy}")
    
    # Save results
    output_file = Path(f"output/{ticker}_opensearch_test_results.json")
    ...
    print_section("TEST COMPLETE", f"Successfully tested VectorDB with OpenSearch for {ticker}")
    print("\n[SUCCESS] All steps completed successfully!")
    print(f"  - Stored {stored_count} company embeddings in OpenSearch")
